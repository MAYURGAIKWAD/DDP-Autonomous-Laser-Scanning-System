...
def scanner_robot_DA():
    ...
    while curr_x_dec < diff:
        ...
        ret = llt.get_actual_profile(hLLT, profile_buffer, len(profile_buffer), llt.TProfileConfig.PROFILE,
                                ct.byref(lost_profiles)) #2d profile from scanner 
	...
	###### timestamp code end         
        ...
        client.read('$POS_ACT.A', debug=False),client.read('$POS_ACT.B', debug=False),client.read('$POS_ACT.C', debug=False)] # can be removed
        t_pose = datetime.datetime.now()
        t_pose_stamp = t_pose.timestamp()

        #lst2 = [lst1[i].decode('utf-8') for i in range(6)]
        time_scanner_arr.append(np.array(t_scan_stamp))
        scanner_profile_data.append(np.array(x))
        scanner_profile_data.append(np.array(z))

        time_pos_arr.append(np.array(t_pose_stamp))
        robot_data.append(np.array(lst1))
        current_x = client.read('$POS_ACT.X', debug=False) # use current x only u at one position
        curr_x_dec = float(current_x.decode('utf-8'))
...
df=pd.DataFrame(time_scanner_arr)
df.to_csv('time_scanner_arr.csv', index=False, header=False)

df=pd.DataFrame(time_pos_arr)
