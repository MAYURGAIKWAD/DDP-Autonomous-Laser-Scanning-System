...
def scanner_robot_DA():
    global curr_x_dec
    print("dummy read to avoid Nonetype - ", type(client.read('$POS_ACT.X', debug=False)))
    while curr_x_dec < diff:
        print("difference : ",(diff-curr_x_dec ))
...
def robot_DA():
    ...
    while curr_x_dec < diff:
        ...
        r_loop_start=datetime.datetime.now()
        r_loop_start_stamp=r_loop_start.timestamp()
        ...
        robot_data.append(np.array([r_loop_start_stamp, current_x,current_y, current_z]))

        curr_x_dec=current_x
