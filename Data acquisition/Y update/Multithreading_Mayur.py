...
def scanner_robot_DA():
    ...
    while curr_y_dec < diff:
        ...
        #print("current x _dec = ", current_x_dec, "current x = ", current_x)
        #current_x = client.read('$POS_ACT.X', debug=False)
        #curr_x_dec = float(current_x.decode('utf-8'))
        
        
        #current_x,current_y, current_z= [client.read('$POS_ACT.X', debug=False).decode('utf-8'), client.read('$POS_ACT.Y', debug=False).decode('utf-8'),client.read('$POS_ACT.Z', debug=False).decode('utf-8')]
        #curr_x_dec = float(current_x.decode('utf-8'))
        #lst1 = [client.read('$POS_ACT.X', debug=False), client.read('$POS_ACT.Y', debug=False),client.read('$POS_ACT.Z', debug=False)]
        #lst1=[current_x,current_y, current_z]
        
        
        #print("X,y z values are: ",current_x,current_y, current_z)

        #client.read('$POS_ACT.A', debug=False),client.read('$POS_ACT.B', debug=False),client.read('$POS_ACT.C', debug=False)]
        #lst2 = [lst1[i].decode('utf-8') for i in range(6)]

        #curr_x_dec = float(lst1[0].decode('utf-8'))
        #curr_x_dec=float(current_x)
        #print("current x _dec = ", current_x_dec, "current x = ", current_x)
        ...
        t_scan_arr.append(np.array(t_scan_stamp))
        scanner_profile_data.append(np.array(x))
        scanner_profile_data.append(np.array(z))
    ...
    #threadLock.release()
    print("difference between values: ", (diff-curr_y_dec))
