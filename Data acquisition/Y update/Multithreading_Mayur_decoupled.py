...
current_y = client.read('COM_E6POS.Y', debug=False)                 ## After scanning(Y) last segment, it won't move in X
curr_y_dec = float(current_y.decode('utf-8'))
print(curr_y_dec)
diff = curr_y_dec + 15
...
def scanner_robot_DA():
    global curr_y_dec
    print("dummy read to avoid Nonetype - ", type(client.read('$POS_ACT.Y', debug=False)))
    while curr_y_dec < diff:
        # print("difference : ",(diff-curr_y_dec ))
        # s_loop_start=datetime.datetime.now()
        # s_loop_start_stamp=s_loop_start.timestamp()
        ...
        t_scan_arr.append([t_scan,t_scan_stamp])
        scanner_profile_data.append(np.array(x))
        scanner_profile_data.append(np.array(z))
       
        
        #del lst2[:]
        # s_loop_stop=datetime.datetime.now()
        # s_loop_stop_stamp=s_loop_stop.timestamp()
        # s_loop_time.append([s_loop_start_stamp,s_loop_stop_stamp])
...
def robot_DA():
    ...
    while curr_y_dec < diff:
        ...
        current_x = float(client.read('$POS_ACT.X', debug=False).decode('utf-8'))
        current_y = float( client.read('$POS_ACT.Y', debug=False).decode('utf-8'))
        current_z = float(client.read('$POS_ACT.Z', debug=False).decode('utf-8'))

        t_pose_arr.append(np.array(t_pose_stamp))
        robot_data.append(np.array([current_x,current_y, current_z]))

        curr_y_dec=current_y
        ...
        #del lst2[:]
        # r_loop_stop=datetime.datetime.now()
        # r_loop_stop_stamp=r_loop_stop.timestamp()
        # r_loop_time.append([r_loop_start_stamp,r_loop_stop_stamp])



def actuation():
    current_y = client.read('COM_E6POS.Y', debug=False)                 ## After scanning(Y) last segment, it won't move in X
    curr_y_dec_1 = float(current_y.decode('utf-8'))
    diff = curr_y_dec_1 + 15
    client.write("COM_E6POS.Y",str(diff), debug=False)
    distance_y = 15
    #velocity = (float(Vel_dec)/100)*18
    #time.sleep(abs(distance_y/velocity))
...
t3 = threading.Thread(target = robot_DA)

# adding lock to help synchronise over cpu
...
df=pd.DataFrame(t_pose_arr)
df.to_csv('time_pos_arr.csv', index=False, header=False)

df=pd.DataFrame(t_scan_arr)
df.to_csv('time_scanner_arr.csv', index=False, header=False)

#np.savetxt('Multi_threading_robot_data.csv', robot_data, delimiter=",")    

# df=pd.DataFrame(s_loop_time)
# df.to_csv('Multi_threading_scanner_loop_time_data.csv', index=False, header=False)

# df=pd.DataFrame(r_loop_time)
# df.to_csv('Multi_threading_robot_loop_time_data.csv', index=False, header=False)
