...
####		####	 why are we doing this reading and then witing again
...
pos = client.read("$POS_ACT", debug=False)  #POs_act and COM_E6_POS have different values
...
####    	#### Why are we taking COME6 POS here and not COME6ACT
...
print('curr_x_dec', curr_x_dec)
diff = curr_x_dec + 40  # to be moved +40 in X
#print ("type of current_x.decode(): ", type(current_x),"value is- ",current_x,  "curr_x_dec:  ", type(curr_x_dec))
...
def scanner_robot_DA():
    ...
    while curr_x_dec < diff:
        ...
        #time.sleep(1000.0/1000000.0) # sleep in microsecond
        #current_x = client.read('$POS_ACT.X', debug=False)
        #current_x = client.read('COM_E6POS.X', debug=False)
        #print("New current X : " , current_x)
        ...
        #lst2 = [lst1[i].decode('utf-8') for i in range(6)]
        robot_data.append([t_pose, t_pose_stamp])
        robot_data.append(np.array(lst1))
        #current_x = client.read('$POS_ACT.X', debug=False)
        s = client.read('COM_E6POS.X', debug=False)
        curr_x_dec = float(current_x.decode('utf-8'))
        delay= t_pose_stamp-t_scan_stamp
        print("delay between the data is:", delay)
        #del lst2[:] 
